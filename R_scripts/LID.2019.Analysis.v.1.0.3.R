...
wd <- setwd("G:/My Drive/S-Lab Analysis/T003_test")
...
for (a in 1:length(BORIS_list)) {
  ...
  merged <- merged[rowSums(is.na(merged[,c(9:29)])) !=21, ] #Drop all rows where NAs are present for subject column 7:27 (check)
...
wd <- setwd("G:/My Drive/S-Lab Analysis")
...
for (i in 1:8) {
  x <- nrow(data[data$M1 == "M1" & data$Zone == i])
  print(x)
}
