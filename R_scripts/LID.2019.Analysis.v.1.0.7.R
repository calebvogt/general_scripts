...
wd <- setwd("C:/Users/caleb/Desktop/dur_test")
...
for (a in 1:length(BORIS_list)) {
  ...
  merged <- merged[rowSums(is.na(merged[,c("M1", "M2", "M3", "M4", "M5","M6","F01","F2","F3","F4","F5","F6","F7","F8","F9","F10","F11","F12","F13","F14","F15")])) !=21, ]
  
  # Drops all duplicated Field.Time rows and keeps rows with STARTS and STOPS
  ...
  #Testing code for looping through individuals to get duration events. 
...
data <- master[master$M1 == subs[1],]



data <- with(master, master[])
