...
############## STEP 4: ###########
## CREATE LIDDELL 2018 MASTER ANALYSIS CSV
...
## CREATE INDIVIDUAL DATA FRAME FOR SPECIFIC TRIAL. FIND AND REPLACE SUBJECT. 
df <- subset(data, (Trial == "T003")) # SUBSET BY TRIAL
df <- with(df,df[day(Field.Time) >= 20 & day(Field.Time) <=21]) # SUBSET BY DATE/DAY IF DESIRED
df <- subset(df, (M6 == 1)) # SUBSET BY SUBJECT
df <- subset(df, Status != "POINT" | is.na(Status)) # Remove all point events, but keeps na values.
df <- subset(df, select = c(Trial, Zone, Field.Time, Subject, Behavior, Status, M6)) #subset columns
df <- subset(df, Subject == "M6" | is.na(Status)) # Only take F01 observations, remove other subjects. 
df <- na.omit(df, cols = c("Field.Time"))
df$Field.Time <- as.POSIXct(df$Field.Time)
df <- as.data.frame(df)

## CREATE INDIVIDUAL STATS DATAFRAME. FIND AND REPLACE SUBJECT
...
s2 <- aggregate(df$M6, list(df$Zone), sum, na.rm=TRUE)
colnames(s2) <- c("Zone", "Duration")
stats <- merge(s1, s2, by = "Zone", all = TRUE)
stats[is.na(stats)] <- 0 
stats$Duration <- stats$Duration/60 # CHANGE DURATION TO MINUTES


## BUBBLE PLOT OF TIME SPENT IN PARTICULAR ZONE 
plot1 <- ggplot(stats, aes(x, y, size = Duration, color = Resource)) +
  ggtitle("Resource Zone Activity Duration (minutes)") +
...
ggsave(filename="plot1.png", plot=plot1, width = 5, height = 4, dpi = 300, units = "in", device='png')


## PLOT INDIVIDUAL MOVEMENT
ggplot(data=df, aes(Field.Time, Zone)) +
  geom_point(na.rm=TRUE, color="red", size=2) +
  ggtitle("Paddock Movement") +
  xlab("Date") + ylab("Zone") +
  scale_x_datetime() +
...
ggplot(stats, aes(Zone, Duration)) +
...
plot2 <- ggplot(df, aes(Field.Time, Zone)) +
