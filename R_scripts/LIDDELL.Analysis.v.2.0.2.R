...
## CREATE DATAFRAME FOR PLOTTING AN INDIVIDUAL'S STATE EVENTS. FIND AND REPLACE SUBJECT  BETWEEN...     ## HERE ## 
...
df <- subset(df, Status != "POINT" | is.na(Status)) # REMOVE ALL POINT EVENTS EXCEPT WHEN STATUS IS NOT NA
...
stats[is.na(stats)] <- 0 
stats$Duration <- stats$Duration/60 # CHANGES DURATION TO MINUTES                       ...AND ## HERE##
