...
#remove duplicates
dur_df <- dur_df[!duplicated(dur_df[c("Field.Time")]),]
...
#dur_df <- subset(dur_df, dur_df$bout_status != "DELETE") (this does not work)
...
# large negative numbers result of start/stop rows coming from different zones! 
...
#Add START and STOP times to data frame (ERROR)
for (i in 2:nrow(dur_df)-1) {
  if(difftime(dur_df$Field.Time[i+1], dur_df$Field.Time[i], units="secs") > 30) {
    if(difftime(dur_df$Field.Time[i+2], dur_df$Field.Time[i+1], units="secs") > 30) {
      if(is.null(dur_df$bout_status[i-1])) {
        dur_df$bout_status[i] <- "STOP"
        dur_df$bout_status[i+1] <- "SINGLE"
        dur_df$bout_status[i+2] <- "START"
      } else {
        if(!is.null(dur_df$bout_status[i-1])) {
            dur_df$bout_status[i] <- "SINGLE"
      }
    }
    } else {
      dur_df$bout_status[i] <- "STOP"
...
for(i in 2:(nrow(dur_df)-1)){
  ...
  if (difftime(dur_df$Field.Time[i+1], dur_df$Field.Time[i], units="secs") >= 30) {
     #print(paste("jumpcounter=",jumpflag))
      jumpflag<-jumpflag+1
    if(difftime(dur_df$Field.Time[i], dur_df$Field.Time[i-1], units="secs") >= 30){

      tempframe <- dur_df[i,,drop=FALSE]
