...
# 2. Create Liddell Movement Animation with plot of the arena. 
...
# ADD XY COORDINATE COLUMNS FOR EACH ZONE TO DATAFRAME
# data$x <- ifelse(grepl("1", data$antenna.id), "20", 
#                    ifelse(grepl("2", data$antenna.id), "50",
#                           ifelse(grepl("3", data$antenna.id), "20",
#                                  ifelse(grepl("4", data$antenna.id), "50",
#                                         ifelse(grepl("5", data$antenna.id), "20",
#                                                ifelse(grepl("6", data$antenna.id), "50",
#                                                       ifelse(grepl("7", data$antenna.id), "20",
#                                                              ifelse(grepl("8", data$antenna.id), "50",
#                                                                     "none"))))))))
# 
# 
# data$y <- ifelse(grepl("1", data$antenna.id), "25", 
#                    ifelse(grepl("2", data$antenna.id), "25",
#                           ifelse(grepl("3", data$antenna.id), "65",
#                                  ifelse(grepl("4", data$antenna.id), "65",
#                                         ifelse(grepl("5", data$antenna.id), "105",
#                                                ifelse(grepl("6", data$antenna.id), "105",
#                                                       ifelse(grepl("7", data$antenna.id), "145",
#                                                              ifelse(grepl("8", data$antenna.id), "145",
#                                                                     "none"))))))))
# 
...
for(i in ids[1:length(ids)]) {
  ...
  # 
  # plot(p1)
  # ggsave(filename=paste0(current_mouse_info, "_Zone_Movement_Scatter.png"), plot=p1, width = 5, height = 4, dpi = 300, units = "in", device='png')
  # 
  # #############
  ...
  #   geom_line(na.rm=TRUE, color="red", size=2) +
  #   ggtitle(paste0(current_mouse_info, " Zone Movement")) +
  ...
  #         
  # # SAVE THE ANIMATION
  # anim_save(filename=paste0(current_mouse_info, "_Zone_Movement_animation.gif"),animation = p2a)

  #############
## PLOT 3 + 3A: ANIMATION OF RFID POINTS AND MOVEMENT FOR FULL TRIAL
  p3 <- ggplot(move_df, aes(x, y)) +
    ggtitle(paste0(current_mouse_info, " Field Zone Movement")) +
  ...
  # SAVE THE ANIMATION
  anim_save(filename=paste0(current_mouse_info, "_Field_Zone_Movement_animation.gif"),animation = p3a,
