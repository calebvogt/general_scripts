...
# TrackingROIs.R from Rusty, Caleb edit on 4.23.2018 -----------------------------------------------


setwd("G:/2018 Data/IR Trial Tracking/dark_social_analysis")
source("MouseFunctions.R", chdir = FALSE)


library(MASS)


setwd(dir<-"G:/2018 Data/IR Trial Tracking/dark_social_analysis") #sets working directory (setwd), and new variable (dir) corresponding to parent directory


directories <- list.dirs(dir, recursive = TRUE)#list.dirs lists directories 
directories <- directories[-c(1:6)]#removes "self" directory, extra files folder, and triaged T001
...
pix.cm
AssociatingDistance.pixels <- 86 # 4.2924 pixels/cm. 20cm association distance = Define pixel-distance that corresponding to an 'association'
...
for (zz in 1:length(directories)){
  ...
  FolderInfo <- strsplit(FolderInfo,"/")[[1]][length(strsplit(FolderInfo,"/")[[1]])] #????? ASK RUSTY ABOUT STRSPLIT
  ...
  if(length(location.names)>0){ #Only if in the right kind of folder, i.e. containing ...fixed.csv files, run the rest, otherwise, skip
    ...
    Roifiles <- list.files(directories[zz], full.names = TRUE,pattern="-region.csv")#refine to pull roi csvs file paths. 
    Roifilenames<-list.files(directories[zz],full.names = FALSE,pattern="-region.csv")#refine to pull roi csvs file names.... 
    ...
    for(Caleb in 1:inds){
      ...
      if(Caleb>1){  ########## ASK RUSTY ABOUT THIS SECTION OF CODE
